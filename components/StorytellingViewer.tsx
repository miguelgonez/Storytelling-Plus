...
export const StorytellingViewer: React.FC<StorytellingViewerProps> = ({ pages }) => {
      ...
      <div className="flex flex-col lg:flex-row gap-8 bg-white rounded-3xl shadow-2xl overflow-hidden p-6 border-4 border-slate-900">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex justify-end mb-2 px-2">
            <img
              src={`${import.meta.env.BASE_URL}sas-logo.png`}
              alt="Servicio Andaluz de Salud"
              className="h-10 md:h-12 w-auto object-contain"
            />
          </div>
          <div className="flex-1 flex justify-center items-center bg-slate-100 rounded-xl p-4 min-h-[500px] relative overflow-hidden">
            <img 
            ...
            />
            <div className="absolute bottom-3 right-3 px-3 py-1 rounded-full bg-white/85 text-slate-800 text-sm font-semibold border border-slate-200 shadow-sm">
              Página {pages[selectedPage].pageNumber}
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-1/3 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4 border-b-2 border-slate-200 pb-2">
              <h2 className="text-3xl font-bold font-storytelling text-blue-600">
                Página {pages[selectedPage].pageNumber}
              </h2>
              <span className="text-slate-400 font-bold">{selectedPage + 1} / {pages.length}</span>
            </div>
            
            <div className="prose prose-lg mb-8 text-slate-700 leading-relaxed font-storytelling">
